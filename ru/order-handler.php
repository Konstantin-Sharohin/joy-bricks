<?php
	...
	function createOrderText($order, $cart_total_price, $address_to_send) {
		...
		$text .=  '	<tr>
	  					<th rowspan="3">Адрес&nbsp;отправки</th>
  					</tr>
					<tr>
	...
	function save_log($id, $new_id, $email, $dbConnect, $error) {
		...
		$get_order_id = "SELECT id FROM ru_users WHERE email = '{$email}'";
		$get_order_id_result = mysqli_query($dbConnect, $user_id);
		list($order_id) = mysqli_fetch_array($get_order_id_result);

		$add_new_log = "INSERT INTO ru_error_logs ('user_id', 'order_id', 'error_log') VALUES ('{$user_id}', '{$order_id}', '{$error}')";
	...
	$get_user_id = "SELECT id FROM ru_users WHERE email = '{$email}'";
	...
	if (mysqli_num_rows($get_user_id_query_result) > 0) {
		...
		$add_order = "INSERT INTO ru_orders (user_id, title, code, price, quantity, payment_amount) VALUES ";
	...
	//Otherwise add a new user and then the order
		...
		$add_new_user = "INSERT INTO ru_users (email, phone, address, first_name, last_name) VALUES ('{$email}', '{$phone}', '{$address_to_send}', '{$first_name}', '{$last_name}')";
		...
		$get_new_user_id = "SELECT id FROM ru_users WHERE email = '{$email}'";
		...
		$add_new_order = "INSERT INTO ru_orders (user_id, title, code, price, quantity, payment_amount) VALUES ";
