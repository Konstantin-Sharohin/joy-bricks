...
<main class="main-container">
	...
	<h3 class="categories-title">Категории</h3>
		<ul class="list">
			<?php
			$query_all_categories = 'SELECT id, category FROM ru_categories ORDER BY category';
	...
	<section class="intro">
		<?php
			if (filter_var($_GET['id'], FILTER_VALIDATE_INT, array('min_range' => 1))) {
				$cat_id = $_GET['id'];
				$query_some_products = 'SELECT id, title, category, photo, price, code FROM ru_products WHERE category_id = ' . $cat_id . ' ORDER BY id';
