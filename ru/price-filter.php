		...
		function query($price_range, $price_sort, $dbConnect) {
			...
			if ($price_range != null && $price_sort == null) {
            	if (filter_var($price_range, FILTER_VALIDATE_INT)) {
            		$query_some_products = 'SELECT id, title, category, photo, price, code FROM ru_products WHERE price <=' . $price_range . ' ORDER BY id';
			...
			elseif ($price_sort != null && $price_range != null) {
					if ($price_sort == "1") {
						$query_some_products = 'SELECT id, title, category, photo, price, code FROM ru_products WHERE price <=' . $price_range . ' ORDER BY price ASC';
						$result_some_products = mysqli_query($dbConnect, $query_some_products);
					}
					elseif ($price_sort == "0") {
						$query_some_products = 'SELECT id, title, category, photo, price, code FROM ru_products WHERE price <=' . $price_range . ' ORDER BY price DESC';
						$result_some_products = mysqli_query($dbConnect, $query_some_products);
					}
			}
