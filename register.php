...
// Страница регистрации сайта
// Этот сценарий отображает и обрабатывает регистрационную форму
// Код этого сценария начал создаваться в главе 4

// Требуется подключение файла конфигурации перед выполнением кода PHP для управления отображением сообщений об ошибках
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	// Проверка логина
	if (preg_match('/^[A-Za-zА-Яа-я0-9]{2,45}$/i', $_POST['username'])) {
	...
	if (empty($reg_errors)) { // если все хорошо...
		...
		if ($rows === 0) { // проблем не возникло
			...
			// Изменение после добавления PayPal!
