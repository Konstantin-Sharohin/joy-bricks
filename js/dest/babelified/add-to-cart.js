...
(function (window) {
  ...
  joyBricks.addToCart = function () {
    ...
    } else console.log("Method 'addEventListener' cannot be applied"); //setting quantity in data attribute for each item
    ...
    function action(event) {
      var selectedIconClass = event.target.className,
      ...
      if (selectedIconClass == "fas fa-cart-arrow-down") {
        ...
        var has_title = false,
            _i = 0,
            _cart_array_length = cart_array.length;

        if (cart_array && _cart_array_length > 0) {
          for (_i; _i < _cart_array_length; _i++) {
            if (current_item.title == cart_array[_i].title) {
              has_title = true;
              current_item.quantity = cart_array[_i].quantity;
            }
          }
        }
